...
```{r}
setwd("/Users/javipatron/Documents/MEDS/Courses/eds230/classes/ESM232_Examples/Rmarkdown")
```
...
# first sum by month
tmp = sager %>% group_by(wy, month) %>% summarize(model = sum(model), 
                                                  obs = sum(obs))
...
as_tsibble(sager) |> 
model(classical_decomposition(model, type = "additive")) |> 
components() |> 
autoplot() +
labs(title = "title")
...
sager$date <- as.Date(paste(sager$year, sager$month, "1", sep="-"), format="%Y-%m-%d")

# Create a new column for quarters
sager$quarter <- quarters(sager$date)
...
sager %>%
  ...
  pivot_longer(cols = c(mean_obs, mean_model),
               names_to = "type",
               values_to = "values") %>%
  ggplot(aes(x = quarter, y = values, group = type, color = type)) +
  geom_line() +
  geom_point() +
  facet_wrap(~ wy, nrow = 2) +
  scale_color_discrete(name = "Water Year") +
  labs(title = "Mean Observed vs. Modeled by Quarter and Water Year",
       x = "Quarter",
       y = "Mean Value")
...
compute_lowflowmetrics(m=sager$model,
                       o=sager$obs, 
                       month=sager$month, 
                       day=sager$day, 
                       year=sager$year, 
